...
export class CacheMetrics {
    ...
    private static metricBlock(
        name: string,
        help: string,
        type: string,
        samples: [labelPart: string, value: number][],
    ): string {
        return [
            `# HELP ${name} ${help}`,
            `# TYPE ${name} ${type}`,
            ...samples.map(([labelPart, value]) => `${name}${labelPart} ${value}`),
            "",
        ].join("\n");
    }
    ...
    toPrometheus(): string {
        ...
        return [
            CacheMetrics.metricBlock("mvt_cache_l1_hits_total", "Total number of L1 cache hits", "counter", [["", s.l1Hits]]),
            CacheMetrics.metricBlock("mvt_cache_l1_misses_total", "Total number of L1 cache misses", "counter", [["", s.l1Misses]]),
            CacheMetrics.metricBlock("mvt_cache_l2_hits_total", "Total number of L2 cache hits", "counter", [["", s.l2Hits]]),
            CacheMetrics.metricBlock("mvt_cache_l2_misses_total", "Total number of L2 cache misses", "counter", [["", s.l2Misses]]),
            CacheMetrics.metricBlock("mvt_cache_bypass_total", "Total number of cache bypass requests", "counter", [["", s.bypassCount]]),
            CacheMetrics.metricBlock("mvt_cache_set_success_total", "Total successful cache writes", "counter", [
                ['{level="L1"}', s.setSuccessL1],
                ['{level="L2"}', s.setSuccessL2],
            ]),
            CacheMetrics.metricBlock("mvt_cache_set_error_total", "Total failed cache writes", "counter", [
                ['{level="L1"}', s.setErrorL1],
                ['{level="L2"}', s.setErrorL2],
            ]),
            CacheMetrics.metricBlock("mvt_cache_get_error_total", "Total cache read errors", "counter", [
                ['{level="L1"}', s.getErrorL1],
                ['{level="L2"}', s.getErrorL2],
            ]),
            CacheMetrics.metricBlock("mvt_cache_singleflight_total", "Total single-flight executions", "counter", [["", s.singleFlightExecutions]]),
            CacheMetrics.metricBlock("mvt_cache_singleflight_waiters_total", "Total requests coalesced via single-flight", "counter", [["", s.singleFlightWaiters]]),
            CacheMetrics.metricBlock("mvt_cache_compute_total", "Total database tile computations", "counter", [["", s.computeTotal]]),
            CacheMetrics.metricBlock("mvt_cache_compute_duration_seconds", "Total time spent computing tiles from PostGIS", "counter", [["", s.computeTotalDurationSeconds]]),
            CacheMetrics.metricBlock("mvt_cache_l1_size_bytes", "Current memory usage of L1 cache in bytes", "gauge", [["", s.l1SizeBytes]]),
            CacheMetrics.metricBlock("mvt_cache_l1_items", "Current number of items in L1 cache", "gauge", [["", s.l1Items]]),
            CacheMetrics.metricBlock("mvt_cache_valkey_latency_seconds", "Total Valkey operation latency in seconds", "counter", [["", s.valkeyLatencyTotalSeconds]]),
        ].join("\n");
    }
